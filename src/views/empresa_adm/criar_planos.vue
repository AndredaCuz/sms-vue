  ...
  <div class="create-plan-page fade-in">
    ...
    <div class="page-header">
      <div class="header-content">
        <router-link to="/admin/planos" class="btn-back">
          <i class="fas fa-arrow-left"></i>
        </router-link>
        <div class="header-text">
          <h1 class="page-title">
            <i class="fas fa-crown"></i>
            {{ modoEdicao ? 'Editar Plano' : 'Criar Novo Plano' }}
          </h1>
          <p class="page-subtitle">
            Configure os detalhes e recursos do plano
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="main-column">
        <form @submit.prevent="salvarPlano">
          ...
          <div class="form-actions">
            <router-link to="/admin/planos" class="btn btn-cancel">
              <i class="fas fa-times"></i> Cancelar
            </router-link>
            <button class="btn btn-save" type="submit" :disabled="isSubmitting">
              <i class="fas fa-spinner fa-spin" v-if="isSubmitting"></i>
              <i class="fas fa-check" v-else></i>
              {{ isSubmitting ? 'Salvando...' : (modoEdicao ? 'Atualizar Plano' : 'Criar Plano') }}
            </button>
          </div>
        </form>
      </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="tips-list">
              <div class="tip-item">
                <i class="fas fa-check-circle"></i>
                <p>Escolha um nome claro e descritivo para o plano</p>
              </div>
              <div class="tip-item">
                <i class="fas fa-check-circle"></i>
                <p>Defina limites realistas baseados nas necessidades dos clientes</p>
              </div>
              <div class="tip-item">
                <i class="fas fa-check-circle"></i>
                <p>Liste todas as funcionalidades incluídas de forma clara</p>
              </div>
              <div class="tip-item">
                <i class="fas fa-check-circle"></i>
                <p>Planos inativos não aparecem para novas assinaturas</p>
              </div>
            </div>
          </div>
        </div>
      ...
      try {
        ...
        if (this.modoEdicao) {
          response = await axios.put(
            `https://api.devsms.online/api/v1/admin/plans/${this.$route.params.id}`,
            payload,
            {
              headers: {
          ...
          response = await axios.post(
            'https://api.devsms.online/api/v1/admin/plans',
            payload,
            {
              headers: {
                'Authorization': `Bearer ${token}`,
        ...
        alert(this.modoEdicao ? '✅ Plano atualizado com sucesso!' : '✅ Plano criado com sucesso!');
        this.$router.push('/admin/planos');
        ...
        if (error.response?.status === 401 || error.response?.status === 403) {
          ...
          alert('Erro ao salvar plano. Por favor, tente novamente.');
        }
      } finally {
    ...
    async fetchPlano() {
      ...
      try {
        ...
        this.form = {
          ...
          price_monthly: plano.price_monthly,
